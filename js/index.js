  ...
  function renderDataTable() {
    ...
    if (underlying == 1) {
      ...
      async function dataLoader() {
        /*let datasources = await worksheet.getDataSourcesAsync();
        let datasource = datasources[0];*/

        let sumdata = await worksheet.getSummaryDataAsync({ maxRows: max_no_records });
        ...
        var fields = []
        column_names.map(function (column, index) {
          let reg = /(?:([a-zA-Z\s]+)+\(+)*([^\)]*)\)*/;
          let reg_column = column.match(reg);
          let caption = reg_column[2]
          let aggregateFunc = reg_column[1]
          let field = {name: index, caption: caption}
          if (aggregateFunc !== undefined) {
            field.aggregateFunc = aggregateFunc.toLowerCase();
          }
